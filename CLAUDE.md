...
## CSE記事の切り出し前（2026-08-06 〜 09-02 測定 / 2026-09-05 実施）

`/eiken-cse-score` を新設し、`/eiken-daigaku-riyou` から CSE 解説を移した。
効果判定はこの数値と比較する。

| キーワード | 月間検索数 | 切り出し前の順位 | 表示 |
|---|---:|---|---:|
| 英検 CSEスコア | 5,000 | 圏外 | 0 |
| 英検 CSEスコア 目安 | 500 | 圏外 | 0 |
| 英検cseスコア 大学 | — | 22.0位 | 4 |
| 英検cseスコア 大学受験 | — | 26.3位 | 3 |
| cse2200 | — | 10.7位 | 6 |

**カニバリの実害チェック用**（既存記事が失ってはいけないもの）

| キーワード | 切り出し前の順位 | 表示 |
|---|---|---:|
| 大阪公立大学 英検 加点 | 1.0位 | 1 |
| 英検準2級プラス | 2.5位 | 2 |
| 英検 不合格 スコア 使える大学 2026 | 9.5位 | 25 |
| cse2200 | 10.7位 | 6 |
| 英検利用できる大学 2026 国立 | 10.7位 | 44 |

`/eiken-daigaku-riyou` は約100行を削減しているため、これらの順位が
下がっていないかを2〜4週後に確認すること。下がっていれば要約が薄すぎる可能性がある。

## 国公立記事の切り出し前（2026-08-13 〜 09-09 測定 / 2026-09-12 実施）

`/eiken-kokkouritsu-daigaku` を新設し、`/eiken-daigaku-riyou` の第7章
「国公立大学の英検利用」を移した。効果判定はこの数値と比較する。

**狙うクエリ**（国公立系が10〜16位で足止め・クリック1件）

| クエリ | 表示 | 順位 |
|---|---:|---:|
| 英検利用できる大学 2026 国立 | 54 | 10.5位 |
| 英検利用できる大学 国公立 | 39 | 15.9位 |
| 英検 利用できる大学 2026 国 公立 | 15 | 9.8位 |
| 英検 利用できる大学 国 公立 | 7 | 12.3位 |
| 共通テスト英検 利用できる大学 2026 | 5 | 13.4位 |

**カニバリの実害チェック用**（既存記事が失ってはいけないもの）

| クエリ | 順位 | 表示 |
|---|---|---:|
| 英検 不合格 スコア 使える大学 2026 | 9.2位 | 32 |
| 英検利用できる大学 2026 | 11.6位 | 29 |
| march 英検利用 2026 | 10.0位 | 8 |
| cse2200 | 10.7位 | 6 |

**個別大学名クエリ**（切り出しで新記事へ移った。移管が成功したかの確認用）

| クエリ | 順位 |
|---|---|
| 大阪公立大学 英検 加点 | 1.0位 |
| 北九州市立大学 英検 加点 2026 | 6.4位 |
| 香川大学 英検利用 | 9.0位 |
| 大分大学 英検利用 | 9.1位 |
| 愛媛大学 英検利用 | 9.7位 |

`/eiken-daigaku-riyou` は第7章で約140行を削減している。個別大学名は新記事へ
移したが、既存記事にも大学名を挙げたポインタ文を1文だけ残してある
（`大阪公立大学 英検 加点` 1.0位を移管期間中に落とさないため）。
2〜4週後に、新記事がこれらを引き継げているかを確認すること。

## ライティング添削記事の公開前（2026-09-13 実施）

`/eiken-writing-tensaku` を新規公開した。既存記事からの切り出しではなく、
**サイトに存在しなかったテーマの新規作成**である点が前2件と異なる。

**狙うキーワード**

| キーワード | 月間検索数 | 競合 | 着手前の順位 |
|---|---:|---|---|
| 英検 ライティング添削 | 500 | 低 | 圏外 |

キーワードプランナーで調べた10語のうち、competition が「低」かつ
サイトに該当コンテンツが存在しない唯一の語だったため選定。

**着手前のサイト内の状態**

- `ライティング添削` の語を含むページ：1件のみ
  （`eiken2-shougakusei.html` の学習スケジュール内「試験2〜3ヶ月前：
  ライティング添削・面接練習を本格化」という1行。解説コンテンツではない）
- 英検ライティングの採点観点・配点を解説したページ：0件

**検索意図の想定**

(a) 添削サービスを探している層と (b) 自分の答案の直し方を知りたい層の
2種類がいる。競合が「低」なのは (b) に応える記事が少ないためと推測し、
記事本体は (b) に充て、(a) には第7章とCTAで応える構成にした。

**事実確認の制約（重要・再作業時は必読）**

この記事の作成時、ネットワークポリシーにより `www.eiken.or.jp` へ直接
アクセスできなかった。採点観点・配点・出題形式は WebSearch で複数の
情報源を突き合わせて確認しており、**公式ページを直接読んで検証したものではない**。
とくに以下は公式で未検証のため、記事中でも「公式で確認」と明示している。

- 級ごとの語数の目安（3級〜準1級）
- 2025年度以降の語数指示の表現変更に関する情報
- 準2級プラスのライティング出題形式（記載を避けた）

公式サイトにアクセスできる環境で、これらの数値を一度確認すること。
