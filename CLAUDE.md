...
| 売場の一覧を設定に取り込む | `npm run netsuper:categories` |
...
- **取り出し方は2通り。** 普通のサイトは画面から読む。画面をキャンバスに描画する
  アプリ（twidy など）は、アプリがサーバから受け取った JSON を記録して読む。
  `items.csv` の `source` 列（`dom` / `api`）で見分ける
...
- **店頭価格との突き合わせは商品名の類似度。** 一致度が低い行は比較していない。
  ...
  出力は判定ではなく**確認するための候補**。差が極端な行は誤マッチを疑う
...
商品カードの自動判定、SPA（`#/` で画面を切り替えるサイト）の遷移、
受信データからの商品抽出を、合成 HTML と localhost の一時サーバで検証する。ネットスーパーには接続しない
