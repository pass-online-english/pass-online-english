...
検索ボリュームを知るには、Google キーワードプランナー（無料。ただし広告費を
使っていないと「100〜1000」のような幅表示になる）や有料ツールが必要です。
このツールでは取得していません。

ただし既存記事の改善フェーズでは、推定ボリュームより
**すでに表示されているクエリの実測**のほうが確実です。
新規記事のテーマ選定でボリューム調査が必要になったときに検討してください。
