...
`https://twidy.jp/#/…` のようにハッシュで画面を切り替えるサイトでは、
URL のハッシュだけを変えても**ページが読み直されない**ため、
そのまま巡回すると前のカテゴリの内容を拾ってしまう。

このツールは遷移前後の URL を比べ、ハッシュだけが違う場合は明示的に読み直す。
また、価格らしきテキストが現れるまで最大15秒待つ（JS の描画待ち）。
