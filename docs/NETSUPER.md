...
**同じ日に何度実行しても積み上がる。** 「豆腐だけ見て閉じる → あとで残りの売場を見る」
という分割ができる。同じ商品が再度出てきたら、新しいほう（最後に見た価格）を採る。
...
商品名が途中で切れている（レシートから起こした場合など）と、別商品に
当たることがある。差が不自然に大きい行は誤マッチを疑う。

```bash
npm run netsuper:diff -- --min-score 0.8   # 名前の一致を厳しくする（既定 0.6）
```

画面にはネット側の表示名と一致度も出る。一致度 0.8 未満には「要確認」が付く。

出力は**買うものを決めるための候補**であって、判定ではない。
並び順は「商品名の一致が確かなもの」が先。差額の大きい順にすると、
別商品に当たった行が上に来て確認の邪魔になるため。

`buy-online.csv` には次の順で列が入る。上から目で追って、買う・買わないを決める。

| 列 | 見かた |
|---|---|
| 確認 | 「要」= 別商品か内容量違いの可能性。表示名を見る |
| 商品名 / ネット表示名 / 内容量 | 同じ商品か、量は同じかを判断する |
| 店頭価格 / ネット価格 / 差額 | 損得 |
| 一致度 | 1.00 が完全一致 |
