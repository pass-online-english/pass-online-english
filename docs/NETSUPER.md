...
集めた分は**数秒ごとに自動で保存される**ので、ブラウザを閉じても、Ctrl+C で
止めても、強制終了しても消えない。終わり方に気を使わなくてよい。
...
絞りたい場合だけ設定の `apiPattern` に正規表現を書く。

通信の監視は**ブラウザ全体**に対して行う。ページ単位では、Service Worker
（通信を代行する仕組み）を通る通信が見えず、商品を取りに行く通信がまるごと
抜け落ちる。別タブで開かれた売場も同時に拾える。

記録した JSON をそのままファイルに残すことはしない（住所や氏名を含みうるため）。
商品が取り出せなかったときだけ、**項目名と型だけ**を `api-shapes.txt` に書き出す。
...
それでも読み込み中から進まない場合は、**パソコンの Google Chrome** を使う。

```bash
npm run netsuper:capture -- --chrome
```

同梱の Chromium は描画まわりが簡素なため、それで動かないアプリがある。
毎回使うなら設定に `"browserChannel": "chrome"` を入れる。
Chrome で開くのは初回だけログインし直しになる。
